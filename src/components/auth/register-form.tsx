...
export function RegisterForm() {
  ...
  const createAccount = async (event: React.FormEvent) => { event.preventDefault(); setError(""); const parsed = registrationSchema.safeParse(form); if (!parsed.success) { setError(parsed.error.issues[0]?.message ?? "กรุณาตรวจสอบข้อมูล"); return; } setLoading(true); try { await services.auth.register(parsed.data); setStep(2); } catch(caught) { setError(caught instanceof Error ? caught.message : "สมัครไม่สำเร็จ"); } finally { setLoading(false); } };

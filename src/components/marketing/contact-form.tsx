...
export function ContactForm() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); setSubmitting(true); await new Promise((resolve) => setTimeout(resolve, 650)); setSubmitting(false); setSent(true); void services.notifications.notify({ title: "กรอกข้อมูลเรียบร้อย", message: "กรุณาติดต่อทีมงานผ่าน support@flukex.store" }); };
  if (sent) return <div className="rounded-xl border bg-green-50 p-8 text-center dark:bg-green-950/30"><div className="mx-auto grid size-12 place-items-center rounded-full bg-green-100 text-success"><Send/></div><h2 className="mt-4 text-xl font-bold">กรอกข้อมูลเรียบร้อย</h2><p className="mt-2 text-sm text-muted-foreground">กรุณาส่งรายละเอียดให้ทีมงานที่ support@flukex.store เพื่อให้เราติดต่อกลับ</p><Button variant="outline" className="mt-5" onClick={() => setSent(false)}>แก้ไขข้อมูล</Button></div>;
